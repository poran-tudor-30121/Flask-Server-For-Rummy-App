...
def upload_file():
    try:
        if 'file' not in request.files or 'positions' not in request.form:
            return jsonify({"error": "Invalid input, file or positions missing."}), 400
        ...
        if file and allowed_file(file.filename):
            ...
            result = process_image(file_path, positions)
...
def process_image(image_path,positions):
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    result = loop.run_until_complete(
        process_image_for_some_players(image_path, logger, positions))
    loop.close()
    logger.info(result)
    return result
